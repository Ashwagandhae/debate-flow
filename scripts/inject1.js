...
(async function () {
	...
	const MENU_INPUT_SELECT = 'input.docs-omnibox-input';
	const GOTO_MENU_ITEM_SELECT = '.ac-renderer > div.ac-row';
	/**
	...
	async function initGoto() {
		...
		// 	}, 100);
		// });
		pressKeyShortcut(cell, '/', { altKey: true });
		let menu = /**  @type {HTMLInputElement} */ (document.querySelector(MENU_INPUT_SELECT));
		focusEl(menu);
		menu.value = 'Go to range';
		await wait(1000);
		focusEl(cell);
		pressKeyShortcut(cell, 'Enter');
		await wait(100);
	}
	/**
	 *
